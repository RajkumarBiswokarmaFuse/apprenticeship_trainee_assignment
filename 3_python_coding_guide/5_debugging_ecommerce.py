    ...
    def remove_product(self, product, quantity):
        ...
        if product in self.products:
            if quantity > 0 and self.products[product] >= quantity:
                ...
                print("Invalid quantity value. Quantity must be greater than 0 "
                      "and less than or equal to the existing quantity.")
        else:
...
# Handle the cases where adding or removing products from the cart fails

# Case 1: Negative quantity should not be allowed to add to cart
customer.add_to_cart(product1, -1)  # This will raise an error and pause at the breakpoint.

# Case 2: Non-existent product should not be allowed to remove
customer.remove_from_cart(product2, 3)  # This will raise an error and pause at the breakpoint.
